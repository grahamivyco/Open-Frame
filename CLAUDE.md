...
**Project covers** (`.pj` cards, all grids) pull each project's brand accent onto the corner fold + arrow, and on hover reveal a brand-tinted panel with the project **wordmark in its own font** + a one-line description — driven by the `PROJECTS` map in the script (accent/font/desc per id); text auto-contrasts to accent luminance.
